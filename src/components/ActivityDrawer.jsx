...
const CardExportModal = lazy(() => import('./CardExportModal.jsx'));
...
function DrawerHeader({ activity, onClose, onExport }) {
  const date = activity?.start_date_local
    ? new Date(activity.start_date_local).toLocaleDateString('en-US', {
        weekday: 'short', month: 'short', day: 'numeric', year: 'numeric',
      }).toUpperCase()
    : '—';
  const type = (activity?.sport_type || activity?.type || 'ACTIVITY').toUpperCase();

  return (
    <div className="sticky top-0 z-10 flex items-center justify-between px-6 md:px-8 py-4 border-b border-white/[0.06] bg-[#0a0a0a]">
      <div className="flex items-center gap-3 min-w-0">
        <span
          className="font-mono text-[9px] uppercase tracking-[3px] px-2 py-0.5 border"
          style={{ color: '#FC4C02', borderColor: 'rgba(252,76,2,0.3)', background: 'rgba(252,76,2,0.08)' }}
        >
          {type}
        </span>
        <span className="font-mono text-[9px] text-white/30 uppercase tracking-widest truncate hidden sm:block">
          {date}
        </span>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0 ml-4">
        ...
        {activity && onExport && (
          <button
            onClick={onExport}
            className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest px-3 py-1.5 border border-white/[0.08] text-white/40 hover:border-orange/40 hover:text-orange transition-colors"
          >
            ...
            <span className="hidden sm:block">Share Card</span>
          </button>
        )}
        <button
          onClick={onClose}
          className="flex items-center gap-2 font-mono text-[10px] text-white/30 hover:text-white transition-colors group"
        >
          <span className="hidden sm:block uppercase tracking-widest">ESC</span>
          <span
            className="w-7 h-7 flex items-center justify-center border border-white/[0.08] group-hover:border-orange/40 group-hover:text-orange transition-colors text-base leading-none"
          >
            ✕
          </span>
        </button>
...
function HeroBlock({ activity }) {
  const location = [activity?.location_city, activity?.location_country]
    .filter(Boolean).join(', ').toUpperCase();

  return (
    <div className="px-6 md:px-8 pt-10 pb-8 border-b border-white/[0.04]">
      {location && (
        <div className="font-mono text-[9px] text-white/25 uppercase tracking-[2px] mb-3 flex items-center gap-2">
          <span
      ...
      <h2
        className="font-bold tracking-[-3px] mb-5"
        style={{
      ...
      {/* Orange rule with glow */}
      <div
        className="h-px w-20 mt-6"
        style={{ background: 'linear-gradient(90deg, #FC4C02, transparent)', boxShadow: '0 0 12px rgba(252,76,2,0.5)' }}
      />
    </div>
  );
}

function PrimaryStats({ activity }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 border-b border-white/[0.06]">
      {stats.map(({ label, value, unit, accent }, idx) => (
        <div
          key={label}
          className={`px-6 py-6 ${idx < 3 ? 'border-r border-white/[0.04]' : ''} ${idx >= 2 ? 'border-t border-white/[0.04] md:border-t-0' : ''}`}
        >
          <div className="font-mono text-[9px] text-white/35 uppercase tracking-widest mb-3">{label}</div>
          <div className={`font-mono text-2xl lg:text-3xl font-light tracking-[-1.5px] ${accent ? 'text-orange' : 'text-white'}`}>
            {value}
            {unit && <span className="text-[11px] text-white/35 ml-1">{unit}</span>}
          </div>
        </div>
      ))}
    </div>
...
function RouteMap({ polyline }) {
  ...
  return (
    <div className="border-b border-white/[0.04]" style={{ height: 280 }}>
      <MapContainer
        key={polyline}
        center={center}
        zoom={14}
      ...
      >
        ...
        <CircleMarker
          center={points[0]}
          radius={5}
          pathOptions={{ color: '#FC4C02', fillColor: '#FC4C02', fillOpacity: 1, weight: 0 }}
        />
        {/* End: white dot */}
        <CircleMarker
          center={points[points.length - 1]}
          radius={4}
          pathOptions={{ color: '#ffffff', fillColor: '#ffffff', fillOpacity: 0.75, weight: 0 }}
        />
      </MapContainer>
      <div
        className="absolute bottom-2 right-2 font-mono text-[8px] text-white/20 uppercase tracking-widest pointer-events-none"
        style={{ zIndex: 1000 }}
      >
        © OSM / CARTO
      </div>
...
function SplitsBars({ splits }) {
  ...
  return (
    <div className="px-6 md:px-8 py-8 border-b border-white/[0.04]">
  ...
  return (
    <div className="p-8 space-y-8 animate-pulse">
      <div className="space-y-4">
        <div className="h-2 bg-white/[0.06] rounded w-24" />
        <div className="h-14 bg-white/[0.06] rounded w-3/4" />
        <div className="h-px bg-orange/10 w-16" />
      </div>
      <div className="grid grid-cols-4 gap-px bg-white/[0.04]">
        {[...Array(4)].map((_, i) => <div key={i} className="h-20 bg-[#0a0a0a]" />)}
      </div>
      <div className="h-[280px] bg-white/[0.04] rounded" />
      <div className="space-y-2">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="flex gap-3 items-center">
            <div className="w-4 h-2 bg-white/[0.04] rounded" />
            <div className="flex-1 h-7 bg-white/[0.03] rounded" style={{ width: `${55 + Math.random() * 40}%` }} />
          </div>
        ))}
      </div>
    </div>
...
export default function ActivityDrawer({ activityId, apiGet, onClose }) {
  ...
  return (
    <>
      ...
      {/* Panel — desktop: right drawer (780px), mobile: full-screen */}
      <div
      ...
      >
        {loading ? (
          <>
            <div className="sticky top-0 z-10 flex items-center justify-between px-8 py-4 border-b border-white/[0.06] bg-[#0a0a0a]">
              <div className="font-mono text-[9px] text-white/20 uppercase tracking-[3px] animate-pulse">
                Loading session...
              </div>
              <button onClick={onClose} className="font-mono text-white/30 hover:text-white text-base transition-colors">✕</button>
            </div>
            <Skeleton />
          </>
        ) : error ? (
          <div className="p-8">
            <DrawerHeader activity={null} onClose={onClose} />
            <div className="font-mono text-sm text-white/40 mt-8">
              <span className="text-orange">ERROR //</span> {error}
            </div>
          </div>
        ) : detail ? (
          <>
            ...
            />
            <HeroBlock activity={detail} />
            <PrimaryStats activity={detail} />
            {detail.map?.summary_polyline && (
              <RouteMap polyline={detail.map.summary_polyline} />
            )}
            {detail.splits_metric?.length > 0 && (
              <SplitsBars splits={detail.splits_metric} />
            )}
            <PerformanceChart streams={streams} />
            <SecondaryMetrics activity={detail} />
            <div className="h-16" />
          </>
        ) : null}
      </div>
